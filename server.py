...
def showSummary():
    ...
    ]  # will raise an IndexError: list index out of range if we try to access the [0] index in an empty club list
    if club:
...
def purchasePlaces():
    ...
    if placesRequired > maxPlacesAllowed:
        return render_template(
            ...
            maxPlacesAllowedMsg=maxPlacesAllowedMsg,
        )

    if (
        club_points < placesRequired
    ):  # check if the number of placesRequired does not exceed point balance
    ...
    if (
        competitionPlaces < placesRequired
    ):  # check if the number of placesRequired does not exceed the competition places left
    ...
    )  # update points number after booking placesRequired
    competition["numberOfPlaces"] = int(competition["numberOfPlaces"]) - placesRequired
    flash("Great, booking complete!")
    return render_template(
        "welcome.html", club=club, competitions=competitions, current_date=current_date
    )
